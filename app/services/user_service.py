...
class UserService:
    ...
    async def create(cls, session: AsyncSession, user_data: Dict[str, str], email_service: EmailService) -> Optional[User]:
        try:
            ...
            profile_picture_url = user_data.pop('profile_picture_url', None)
            ...
            new_user.role = UserRole.ADMIN if (await cls.count(session)) == 0 else UserRole.ANONYMOUS
    ...
    @classmethod
    async def update_profile_picture_url(cls, session: AsyncSession, user_id: UUID, url: str) -> Optional[User]:
        try:
            query = update(User).where(User.id == user_id).values(profile_picture_url=url).execution_options(synchronize_session="fetch")
            await cls._execute_query(session, query)
            updated_user = await cls.get_by_id(session, user_id)
            if updated_user:
                session.refresh(updated_user)
